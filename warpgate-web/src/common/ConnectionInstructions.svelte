<script lang="ts">
    ...
    async function loadCertificates() {
        ...
        try {
            ...
            const preferred = preferredId
                ? certificates.find(c => c.credential.id === preferredId)
                : undefined
            const toSelect =
                preferred ??
                certificates.find(c => c.hasLocalKey) ??
                certificates[0]
            if (toSelect) {
    ...
    let selectedOidc = $derived(
        k8sOidcConfigs.find(c => c.name === selectedOidcProvider),
    )
...
{#if targetKind === TargetKind.Http}
    <CopyableTextArea
        label="Access URL"
        value={targetName ? makeTargetURL(opts) : ''}
    />

    {#if ticketSecret}
        Alternatively, set the <code>Authorization</code> header when accessing
        the URL:
    ...
    {#if kubeconfigMode === 'oidc' && k8sOidcConfigs.length > 0}
        {#if k8sOidcConfigs.length > 1}
            ...
            <select
                ...
                bind:value={selectedOidcProvider}
            >
        ...
        <div class="row">
                        ...
                        {#if !selectedCertId || !clientPrivateKeyPem}
                            <InfoBox class="mb-2">
                                {#if certificates.length > 0}
                                    ...
                                    {:else}
                                        The private key for this certificate is
                                        not stored in this browser.
                                    {/if}
                                    The kubeconfig will contain placeholders for
                                    authentication.
                                {:else}
                                    You need a certificate credential to connect
                                    to this target.
                                {/if}
                            </InfoBox>
                            <InfoBox>
                                You can issue a new certificate using the button
                                above and enable the "Store in browser" option
                                to generate a ready-to-use kubeconfig.
                            </InfoBox>
                        {/if}
                    {/if}
                {/if}
            ...
            {#if certificates.length > 0}
                <div class="col-12 col-lg">
                    ...
                    <InfoBox class="mt-3">
                        Save the kubeconfig above to a file (e.g.
                        <code>warpgate-kubeconfig.yaml</code>) and use it with
                        kubectl.
                    </InfoBox>
                </div>
            {/if}

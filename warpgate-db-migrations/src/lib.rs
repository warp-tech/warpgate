...
mod tests {
    ...
    /// The salt is generated by the migration, not by a column default, so an
    /// upgraded installation only ends up with one if the stamping UPDATE
    /// actually ran. A blank salt would leave every stored credential digest
    /// keyed on nothing.
    #[tokio::test]
    async fn migrations_generate_a_credential_digest_salt() {
        set_config_migration_values(ConfigMigrationValues {
            recordings_enable: false,
            recordings_path: "/tmp/warpgate-test-recordings".to_owned(),
            ssh_host_key_verification: SshHostKeyVerificationMode::default(),
        ...
        assert_eq!(
            count(
                &db,
                "SELECT COUNT(*) AS n FROM parameters WHERE credential_digest_salt = ''",
            )
            .await,
            0,
            "the parameters row should have been stamped with a salt",
        );
    }
}

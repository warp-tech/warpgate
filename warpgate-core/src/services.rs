...
use tracing::warn;
use warpgate_common::auth::{AuthResult, AuthState, CredentialDigestSalt, CredentialKind};
use warpgate_common::{
...
pub struct Services {
    ...
    /// Read once at startup: it is generated by the migration that introduces
    /// it and never changes, and re-reading it per digest would put a query on
    /// the path of every approval lookup.
    pub(crate) credential_digest_salt: Arc<CredentialDigestSalt>,
...
/// The installation's credential-digest salt.
///
/// Absent means the parameter row predates the migration that generates one,
/// which cannot happen on a database this binary has migrated — so it is an
/// error rather than a fresh salt: minting one here would silently invalidate
/// every digest the rest of the cluster is matching against.
async fn read_credential_digest_salt(db: &DatabaseConnection) -> Result<CredentialDigestSalt> {
    CredentialDigestSalt::from_stored(&Parameters::Entity::get(db).await?.credential_digest_salt)
        .ok_or_else(|| anyhow::anyhow!("credential digest salt is missing from the parameters"))
}
...
impl Services {
    pub async fn new(
        ...
        let services = Self {
            ...
            credential_digest_salt: Arc::new(read_credential_digest_salt(&db).await?),
            login_protection,
        ...
        services
            ...
            .set_request_sink(ApprovalRequestSink {
                ...
                salt: services.credential_digest_salt.clone(),
            });

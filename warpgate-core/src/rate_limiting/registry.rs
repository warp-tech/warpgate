...
use crate::{State, TargetSessionState, UserSessionState};
...
impl RateLimiterRegistry {
    ...
    pub async fn update_all_user_session_rate_limiters(
        &mut self,
    ...
    pub async fn update_all_target_session_rate_limiters(
        &mut self,
        state: &mut TargetSessionState,
    ) -> Result<(), WarpgateError> {
        let handles = std::mem::take(&mut state.rate_limiter_handles);

        let result = async {
            for handle in &handles {
                self.update_target_rate_limiter(Some(&state.target), handle)
                    .await?;
                self.update_global_rate_limiter(handle).await?;
            }
            Ok(())
...
pub async fn apply_new_rate_limits(
    ...
    let (user_sessions, target_sessions): (Vec<_>, Vec<_>) = {
        let state = state.lock().await;
        (
            state.user_sessions.values().cloned().collect(),
            state.target_sessions.values().cloned().collect(),
        )
    };
    for session_state in user_sessions {
        ...
        registry
            .lock()
            .await
            .update_all_user_session_rate_limiters(&mut session_state)
            .await?;
    }
    for session_state in target_sessions {
        let mut session_state = session_state.lock().await;
        registry
            .lock()
            .await
            .update_all_target_session_rate_limiters(&mut session_state)
            .await?;
    }
    Ok(())
}

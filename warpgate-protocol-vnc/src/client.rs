...
use warpgate_core::{
    AdmittedTarget, DESKTOP_INPUT_CHANNEL_CAPACITY, DesktopEvent, DesktopInput, DesktopRect,
    DesktopState,
};
...
fn spawn_approved_client(
    ...
    let options = admitted.specific_target().options();
    Ok(spawn_client(options.clone(), encodings))
}

...
def test_the_same_endpoint_still_says_what_it_can_say(
    ...
    # Every 5xx carries a reference, so only the reason in front of it tells
    # a caller-authored message from the flattened status text.
    assert not body.startswith("Internal Server Error (reference: "), (
        f"a message written for the caller was flattened to the status text: {body!r}"
    )

...
use warpgate_db_entities::{Parameters, Session as SessionEntity, SessionApprovalRequest};
...
impl Api {
    ...
    async fn api_auth_state(
        ...
        match find_user_approval_row(&ctx, *id).await? {
    ...
    async fn api_reject_auth(
        ...
        resolve_own_approval(&ctx, &id, ApprovalDecision::Rejected).await
    }
}

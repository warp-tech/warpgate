...
pub async fn proxy_normal_request(
    ...
    mut target_close_rx: broadcast::Receiver<()>,
...
async fn copy_client_body(
    ...
    embed_ui: bool,
    target_close_rx: broadcast::Receiver<()>,
    keepalive_guard: Option<SessionKeepaliveGuard>,
) -> Result<()> {
    if embed_ui {
    ...
    let guarded = futures::stream::unfold(
        (body, target_close_rx, keepalive_guard),
        |(mut body, mut target_close_rx, keepalive_guard)| async move {
            tokio::select! {
                item = body.next() => item.map(|item| {
                    (item, (body, target_close_rx, keepalive_guard))
                }),
                _ = target_close_rx.recv() => None,
            }
        },
    );

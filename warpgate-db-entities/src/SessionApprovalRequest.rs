...
/// that can act on it, reads it back from there. The row is deleted once the
/// owner has consumed the decision, when the waiter gives up, when the session
/// ends, or by age-based reaping if the owner died.
///
